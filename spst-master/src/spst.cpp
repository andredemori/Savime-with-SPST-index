...
SPSTTree PrintResultList(SPSTTree root){

    for (auto it = root->resultsearchlist.begin(); it != root->resultsearchlist.end(); ++it)
        printf("%d\n", it->position);
}
...
*/
  ...
  for (size_t i = 0; i < 3; i ++){
    ...
    Filter_spst(1, ">", T, ResultList); // alterar filter para aceitar um double
