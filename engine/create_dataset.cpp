    ...
    if((parameter3 != NULL) && (parameter2->literal_str.compare("___LITERAL___") == 0)) {
        ...
        if (parameter3.get()->literal_str.compare("\"spst-index\"") == 0) {
            ...
            for (int i = 0; i < temp.size() - 3; i++) {
                ...
                T = Insert(i, stod(temp3->literal_str), T);
                //datasetLiteral[i] = stod(temp3->literal_str); //converter para double
                temp2++;
            }
        } else {
            throw std::runtime_error("Invalid indexing structure.");
        }
    }
      ...
      if (!fileFiller) {
        ...
        if (regex_match(filler, range)) {
          ...
          if (ds == nullptr)
            ...
            if((parameter3 != NULL) && (parameter3.get()->literal_str.compare("\"spst-index\"") == 0)){
        ...
        ds->Sorted() = false;
          ...
          if((parameter3 != NULL) && (parameter3.get()->literal_str.compare("\"spst-index\"") == 0)){
              ...
              int read;
              int i = 0;
              //double datasetIndexfile[ds->GetEntryCount()]; //array não pode ter esse tamanho
                 if (file2.is_open()) {
                     while (!file2.eof()) {
